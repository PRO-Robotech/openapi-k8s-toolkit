...
Thank you for contributing to **openapi-k8s-toolkit**.
...
| `npm run test` passes (if applicable)          | ✅ Required    |
...
## 🧩 Factory Components Rule

If your contribution includes a new **Dynamic Factory Component**, then the PR must also include:

- A **Storybook docs-only story** using the existing pattern.
  (See: `/src/components/organisms/DynamicComponents/.../stories.tsx`)

- Example config demonstrating **factory usage**, not standalone usage.

PRs missing Storybook support for new factory components will not be merged.
...
[ ] Storybook story added (if feature is UI/Factory-related)
