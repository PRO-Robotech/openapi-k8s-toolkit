...
describe('seriesToStackedAreaData', () => {
  ...
  test('builds stacked data for a single series', () => {
    ...
    expect(matrixToStackedAreaData(series)).toEqual([
      { timestamp: 1000, c1: 1 },
      { timestamp: 2000, c1: 2 },
    ])
  })
  ...
  test('gracefully handles series with missing data', () => {
    ...
    expect(matrixToStackedAreaData(series)).toEqual([{ timestamp: 1000, nonEmpty: 42 }])
  })
})
