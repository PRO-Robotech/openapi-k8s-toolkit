...
const mkSeries = (idKey: 'pod' | 'container', id: string, base: number, start: number, end: number, step: number) => {
  ...
  return { metric: { __name__: 'container_memory_usage_bytes', [idKey]: id }, values }
}
...
const successHandler = http.get('http://localhost:9090/api/v1/query_range', ({ request }) => {
  ...
  return HttpResponse.json({
    ...
    data: {
      resultType: 'matrix',
      result: [
        mkSeries('pod', 'pod-a', 350_000_000, start, end, step),
        mkSeries('pod', 'pod-b', 900_000_000, start, end, step),
        mkSeries('pod', 'pod-c', 1_450_000_000, start, end, step),
      ],
    },
  })
})
...
const meta: Meta<TArgs> = {
  title: 'Factory/Prometheus Internal/Matrix/ToLine/Multi',
      ...
      <>
        ...
        >
          <div style={{ padding: 16 }}>
            <MatrixToLineMulti range={data.range} formatValue={buildFormatValue(args.formatter)} />
          </div>
        </SmartProvider>
